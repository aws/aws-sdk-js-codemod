  ...
  for (const clientName of CLIENT_NAMES) {
    ...
    source.find(j.ClassDeclaration, { id: { name: clientName } }).forEach((classDeclaration) => {
      ...
      classMethods.forEach((classMethod) => {
        ...
        const defaultInputNames = V2_CLIENT_INPUT_SUFFIX_LIST.map((s) => `${commandName}${s}`);
        if (!defaultInputNames.includes(requestTypeName)) {
        ...
        const defaultOutputNames = V2_CLIENT_OUTPUT_SUFFIX_LIST.map((s) => `${commandName}${s}`);
        if (!defaultOutputNames.includes(responseTypeName)) {
